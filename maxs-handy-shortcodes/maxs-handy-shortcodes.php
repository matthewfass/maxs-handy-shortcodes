...
// Revisions
// 2023-10-04 - added post_title
// 2023-10-04 - refactor acf shortcode
...
/* [post_title id=x] post_title [post_title id=x] returns the title of the post*/
...
/* --------------------------------------------------------------------- */
/* [show_acf_field field="your_custom_field_name" wrapper="span" class="your_custom_class" id="your_custom_id"] */
/* --------------------------------------------------------------------- */
...
function show_acf_field($atts = [])
{
	...
	if (!function_exists('get_field')) {
		return 'Error: ACF get_field function is not available.';
	}

	// Make sure we have a valid field name before trying to fetch
	if (!$attributes['field']) {
		return '';
	}

	$text = get_field($attributes['field']);

	// If we couldn't retrieve the field data, exit early
	if (!$text) {
		return '';
	}

	$classAttr = $attributes['class'] ? ' class="' . esc_attr($attributes['class']) . '"' : '';
	$idAttr = $attributes['id'] ? ' id="' . esc_attr($attributes['id']) . '"' : ' id="' . esc_attr($attributes['field']) . '"';
...
function myplaceholder($atts, $content = null)
{
	...
	if ($text !== "") {
		$text = "&text=" . str_replace(" ", "+", $text) . "+" . $size;
	}
...
/* --------------------------------------------------------------------- */
/* ! post_title [post_title id=x] returns the body of the post*/
/* --------------------------------------------------------------------- */
function post_title($atts, $content = null)
{
	extract(shortcode_atts(array(
		'id' => get_the_id()
	), $atts));
